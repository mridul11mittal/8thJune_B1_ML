...
app=t.Tk()
app.title("Basic calculator") ## App window title
app.geometry('300x300')  ## setting dimensions of calculator app window

val=t.Variable(app)
t.Entry(app,textvariable=val,justify='right',width=30).place(x=50,y=50) ## For displaying screen in calculator.

exp=""


## To concatinate the input from user to evaluate it later
def operate(e):
    global exp
    exp= exp + str(e)
    val.set(exp)
...
## function to clear the expressions after calculations here putting val.set to empty
def clr(x):
    global exp
    exp=""
    val.set("")
...
## Function to evaluate the above concanitated expressions according to the function called.    
def equal(n):
    global exp
    r=str(eval(exp))
    val.set(r)
...
## Creating different buttons for differnt operations and inputs by user    
